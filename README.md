...
This application is designed to be hosted natively on **Vercel** and utilizes **Upstash Redis** for database persistence.
...
2. **Cron Job (`/api/cron`)**: Vercel routinely pings this endpoint once per day (Midnight UTC) on the free Hobby tier. This endpoint probes the live production endpoints of the 4 SmoothSend services, evaluates their responsiveness, and logs their status (Operational, Degraded, or Outage) securely into the Upstash Redis database.
3. **Status API (`/api/status`)**: When a user loads the status page, this endpoint retrieves the past 90 days of daily aggregates from the Redis database, merges it with a live real-time ping for the current day, and serves it to the frontend.
4. **Resiliency**: If the Redis database is not configured or offline, the API automatically falls back to gracefully returning purely live stateless ping results to keep the dashboard visually unbroken.
...
3. **Provision Database**: Go to [Upstash](https://upstash.com/), create a free Redis database, and drop the provided `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` into your Vercel Project Environment Variables.
...
To run locally, copy `.env.example` to `.env.local` and substitute your Upstash Redis REST URL variables:
```bash
npm install
npm run dev
